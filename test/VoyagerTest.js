...
describe("Voyager Contract", function () {
  ...
  before(async function () {
    ...
    GravFactory = await ethers.getContractFactory("Token1");
    xGravFactory = await ethers.getContractFactory("Token2");
    VoyagerFactory = await ethers.getContractFactory("Voyager1");

    NFT = await NFTFactory.deploy();
    Grav = await GravFactory.deploy();
    xGrav = await xGravFactory.deploy();
    Voyager = await VoyagerFactory.deploy(
      NFT.address,
      xGrav.address,
      Grav.address
    );
    ...
    await xGrav
      .connect(owner)
      .increaseAllowance(Voyager.address, parseEther("1000"));
  ...
  describe("Intialization", function () {
    ...
    for (var i = 1; i < 10; i++) {
      rarity.push([
        i,
        1670760,
        "0x91369e121087da8ce3a93a8fb3130ad45da79788fda60bb017eef42ff61fc49d",
      ]);
    }
  ...
  describe("Start Voyage", function () {
    ...
    it("Should update Info mapping", async function () {
      for (var i = 0; i < 4; i++) {
        expect(
          (await Voyager.getStakeInfo(owner.address, i + 1))["tokens"].map(
            (value) => {
    ...
    it("Should update User Staked", async function () {
      for (var i = 0; i < 4; i++) {
        expect(
          (await Voyager.getUserStaked(owner.address)).map((value) => {
            return parseInt(value);
          })
        ).to.include(i + 1);
      }
    ...
    it("Should update contract xGrav balanace", async function () {
      ...
      balance += balance * 0.2;
      expect(await xGrav.balanceOf(Voyager.address)).to.equal(
    ...
    it("Should update Fee Balance", async function () {
      ...
      balance = 0.2 * balance;
      expect(await Voyager.feeBalance()).to.equal(
        parseEther(balance.toString())
      );
    });
  });
  ...
  describe("Recover fees", function () {
    it("Should recover fees", async function () {
      ...
      expect(parseInt(userPreBalance) + parseInt(fees)).to.equal(
        parseInt(userPostBalance)
      );
    });
  ...
  describe("End Early", function () {
    it("Should return token to owner", async function () {
      await expect(Voyager.endEarly([1]))
        .to.emit(Voyager, "Result")
        .withArgs(owner.address, 1, false, 1);
    ...
    it("Should delete mapping", async function () {
      expect(
        (await Voyager.getStakeInfo(owner.address, 1))["tokens"]
      ).to.deep.equal([]);
    });
    it("Should update result", async function () {
      expect((await Voyager.getResult(owner.address, 1))["win"]).to.equal(
        false
      );
      expect(
        (await Voyager.getResult(owner.address, 1))["tokens"].map((value) => {
          return parseInt(value);
        })
      ).to.deep.equal([1, 2]);
    });
  ...
  describe("End on time", function () {
    ...
    it("Should return token to owner", async function () {
      ...
      await expect(Voyager.endVoyage([2]))
        .to.emit(Voyager, "Result")
        .withArgs(owner.address, 2, true, 1);
      for (var i = 3; i < 6; i++) {
        expect(await NFT.ownerOf(i)).to.equal(owner.address);
      }
    });
    ...
    it("Should delete mappings", async function () {
      expect(
        (await Voyager.getStakeInfo(owner.address, 2))["tokens"]
      ).to.deep.equal([]);
    });
    it("Should update result", async function () {
      expect((await Voyager.getResult(owner.address, 2))["win"]).to.equal(true);
      expect(
        (await Voyager.getResult(owner.address, 2))["tokens"].map((value) => {
          return parseInt(value);
        })
      ).to.deep.equal([3, 4, 5]);
    });
  ...
  describe("Fail conditions", function () {
    it("Can't stake someone else's token", async function () {
      await expect(
        Voyager.connect(ace).startVoyage([[11, 12]], [1])
      ).to.be.revertedWith("Not owner");
    });
    it("Amount can't be other than 1 or 2", async function () {
      await expect(Voyager.startVoyage([[13]], [3])).to.be.revertedWith(
        "Invalid price"
      );
    });
    it("Rarity must be set", async function () {
      await expect(Voyager.startVoyage([[13]], [1])).to.be.revertedWith(
        "Rarity not set"
      );
    });
    it("Should not end early if time over", async function () {
      await expect(Voyager.endEarly([4])).to.be.revertedWith(
        "Already completed"
      );
    });
